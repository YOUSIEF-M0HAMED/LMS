                ...
                <div class="chat-body overflow-x-hidden">
                    ...
                    @else
                        @foreach ($oldMessages as $message)
                            @if ($message->sender->is(Auth::guard('teacher')->user()))
                                <div class="row align-items-start justify-content-start flex-row-reverse msg-out">
                                    @if (empty($message->sender->image))
                                        <img class="d-inline-block" src="{{ asset('images/un known person.jpeg') }}" alt="" />
                                    @else
                                        <img class="d-inline-block" src="{{ asset($message->sender->image) }}" alt="" />
                                    @endif
                ...
                <div class="chat-form-container">
                    <form id="chat-form">
                        ...
                        <div class="input-group">
                            <input type="hidden" name="sender_id" value="{{ Auth::guard('teacher')->id() }}">
                            ...
                            <button id="btn-audio-recorder" type="button" class="btn btn-secondary mic-icon border border-right border-1 border-white">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            form.addEventListener('submit', function (event) {
                ...
                const url = '{{ route('teacher.courses.chat.sendMessage') }}';
            ...
            recordButton.addEventListener('click', async () => {
                if (!mediaRecorder || mediaRecorder.state === 'inactive') {
                    ...
                    mediaRecorder.onstop = async () => {
                        ...
                        formData.append('sender_id', '{{ Auth::guard('teacher')->id() }}');
                        formData.append('sender_type', 'teacher');
                        formData.append('course_id', '{{ $courseId }}');

                        const response = await fetch('{{ route('teacher.courses.chat.uploadVoice') }}', {
                            method: 'POST',
                            body: formData
                        });
            ...
            function fetchLatestMessages() {
                ...
                const url = `{{ route('teacher.courses.chat.latestMessages', ['courseId' => '__courseId__']) }}`.replace('__courseId__', courseId);
                ...
                fetch(url)
                    ...
                    .then(messages => {
                        ...
                        messages.forEach(message => {
                            ...
                            if (message.type === 'text') {
                                messageHtml = `
                                    <div class="row align-items-start ${message.sender_id == {{ Auth::guard('teacher')->id() }} && message.sender_type == 'teacher' ? 'flex-row-reverse' : ''} msg-out">
